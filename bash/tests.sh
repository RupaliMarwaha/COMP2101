...
test -w /etc/resolv.conf && echo "/etc/resolv.conf is writable " || echo "/etc/reolv.conf is not writable"
test -x /etc/resolv.conf && echo "/etc/resolv.conf is executable " || echo "/etc/reolv.conf is not executable"
...
[ -d  /tmp -a -w /tmp ] && echo "/tmp is a directory and is writable " || echo "/tmp is not a directory"
[ -d /tmp -a -x /tmp ]  && echo "/tmp is a directory and can be accessed" || echo "/tmp is not a directory"
...
[ $(($firstNumber % 2)) -eq $tester ] && echo "The first number is an even number" || echo "The first number is an odd number"
[ $(($secondNumber % 2)) -eq $tester ] && echo "The second number is an even number" || echo "The second number is an odd number"
[ $(($secondNumber % $firstNumber)) -eq $tester ] && echo "The second number is multiple of first number" || echo "The second number is not a multiple of first f number"
...
[ -n USER ] && echo "The data contained in user is : $USER"
[ -v USER -a -z USER ] && echo "The variable exists but it is empty"
...
read -p "Please key in a string :" a
read -p "Please key in another string :" b
